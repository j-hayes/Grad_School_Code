...
[alpha_part7_low, A_part7_low, b_part7_low, Tn_part7_low]= problem1(10,.5,10);
[alpha_part7_high, A_part7_high, b_part7_high, Tn_part7_high]= problem1(10,1.5,10);
...
ylabel('T Kelvin')
xlabel('z')
title('problem 1 part 7 h=1.5 g=10 N=10');




function [alpha, A, b, Tn, range] = problem1(g,h, N)
syms z;
i = 1:1:10;
wi = sym((1-i).*pi);
lambda = -(wi.^2);
...
while j <= N
    if j > 1
        ...
        b(j) = (-1*(getInnerProduct(g,phi(j)) + Tr*subs(phi(j),0)))/h;
    end
     j = j +1;
end
i =1;
...
j = 1;
    while j <= N 
        if i == j
            A(j,i) = lambda(i) + h*subs(phi(j),0);        
        elseif i == 1
            A(j,i) = h*subs(phi(j),0);          
        else            
            A(j,i) = h*2^(1/2)*subs(phi(j),0);
        end        
        j = j+1;
    end
